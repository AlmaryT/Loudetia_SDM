...
ZA <- ne_countries(country = c('South africa', 'Lesotho', 'Swaziland'), scale = 'medium', returnclass = 'sf') 
ZA
plot(vect(ZA))
# the island in the bottom right corner needs to be removed
e <- ext(15, 33, -35, -22)
ZA <- crop(vect(ZA), e)
plot(ZA)
ZA <- st_as_sf(ZA)
...
spp_sf <- st_as_sf(mask(vect(spp_sf), vect(ZA)))

#### NEW: filter out points in central S.Afr
spp_sf %>% filter(!download_ID %in% c('bien_2148', 'bien_2131', 'bien_1971', 'bien_2116','gbif_1851', 'gbif_1847','gbif_1841','gbif_1734')) -> spp_sf
...
worldclim <- rast("C:/MASTER TCHANA/Loudetia_SDM/data/rast/subsah_afr_cov.tiff")
worldclim <- rast("data/rast/all_env_vars.tif")
names(worldclim)

# Crop and mask
wc_ZA <- worldclim %>% crop(., vect(ZA)) %>% mask(., vect(ZA))
...
vars_sel_fin[[c(3)]] <- vars_sel_fin[[c(3)]]/10
...
# Save model
saveRDS(final_model, 'output/models/ZA_RF_model.RDS')
...
raster::writeRaster(pred, 'data/rast/ZA_Loudetia_simplex_pred_prob.tiff', overwrite = TRUE)
