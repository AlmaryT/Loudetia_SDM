...
subsh <- read_sf('data/vect/sub_saharan_afr.shp')
spp_sf <- st_as_sf(mask(vect(spp_sf), vect(subsh)))
...
worldclim_sel <- worldclim[[sel_vars]]

## Crop and mask
vars_sel_fin <- worldclim_sel %>% crop(., vect(subsh)) %>% mask(., vect(subsh))
...
writeRaster(vars_sel_fin, 'data/rast/subsah_afr_cov_11.tiff', overwrite = TRUE)
...
spp_filt <- occfilt_geo(
  ...
  env_layer = vars_sel_fin,
  prj = crs(vars_sel_fin)
)
...
ggplot() + 
  geom_sf(data = subsh, fill = 'gray90') +
...
saveRDS(final_model, 'output/models/africa_RF_model.RDS')
...
raster::writeRaster(pred, 'data/rast/AFR_Loudetia_simplex_pred_prob.tiff', overwrite = TRUE)
